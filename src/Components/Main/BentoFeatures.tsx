...
const BentoFeatures = () => {
    ...
    return (
        <Box variant='bare' className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-5 auto-rows-[170px] sm:auto-rows-[170px] max-w-5xl w-full self-center mt-10 px-3'>
            {bentoFeatures.map((f, i) => (
                <Reveal
                    key={i}
                    delay={i * 0.06}
                    className={`min-w-0 ${f.big ? 'sm:col-span-2 sm:row-span-2' : ''} ${f.wide ? 'sm:col-span-2' : ''}`}
                >
                    <Box
                        variant='card'
                        className={`p-6 flex flex-col gap-2 h-full min-w-0 transition-all duration-200 hover:-translate-y-1 hover:shadow-lg ${f.big ? 'justify-start' : ''}`}
                    >
                        <div className='w-10 h-10 rounded-xl flex items-center justify-center shrink-0' style={{ backgroundColor: colors.primarySoft }}>
                            <f.icon size={20} color={colors.primary} />
                        </div>
                        <Text className={`font-bold mt-2 break-words ${f.big ? 'text-xl' : 'text-base'}`}>{f.title}</Text>
                        <Text variant='soft' className='text-sm break-words' style={{ fontFamily: fonts.body }}>{f.desc}</Text>
                    </Box>
                </Reveal>
            ))}
        </Box>
